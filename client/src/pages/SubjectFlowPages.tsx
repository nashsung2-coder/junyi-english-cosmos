...
import FirstHarborWelcome from "@/components/FirstHarborWelcome";
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_70%_50%_at_75%_0%,color-mix(in_srgb,var(--accent)_12%,transparent),transparent_68%),#061014] text-foreground">
      ...
      <main className="page-safe-top container max-w-6xl pb-14" aria-label={`${subject.name}${meta.managementLabel}`}>
        <Link href={`/${area === "hall" ? "hall" : area}`} className="tap-target inline-flex items-center gap-2 text-sm text-slate-400 transition-colors hover:text-white"><ArrowLeft className="h-4 w-4" />返回科目選擇</Link>
        <section className="mt-5 overflow-hidden rounded-3xl border border-white/10 bg-white/[.026] p-5 shadow-[0_20px_70px_rgba(0,0,0,.26)] sm:p-8">
          <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-start">
            ...
            <Link href={`/practice/${missionId}`}><Button className="tap-target w-full sm:w-auto" style={{ background: subject.color, color: "#071016" }}><Play className="mr-2 h-4 w-4" />開始{subject.name}任務</Button></Link>
          </div>
